 <div class="previewButton"><img class="previewImg" src="<?php echo base_url('assets/img/IconJob_Rubric.png'); ?>" alt="Preview"></div>
 </div>
 </div>
 ...
 <script type="text/javascript" id="zsiqchat">
 	...
 	$zoho.salesiq = $zoho.salesiq || {
 		widgetcode: "595a3f6e6289264cf3c2dd870089c52785ee3a20d97577fc5e0bc6d236b6d39d0647a20e31232f91924cefe67c201f5a",
 		values: {},
 		ready: function() {}
 	};
 	var d = document;
 			...
 			<div class="modal-footer">
 				<div class="modalFooter">
 					<p><span>Don't have and account? <a href="javascript:void(0);" onclick="return showSignup('signupModal');"> Sign up </a></span><br>
 						<span>Lost your password? <a>Recover password</a></span>
 					</p>
 				</div>
 			</div>
 		</div>
 				...
 				<div class="d-flex flex-column">
 					<form id="signupForm" name="signupForm" method="post" action="" autocomplete="off">
 						<div class="form-group"> <span>Name</span>
 							<input type="text" class="form-control" id="user_name" name="user_name" autocomplete="off" placeholder="">
 							<div id="error_name" class="signup_error"></div>
 						</div>
 						<div class="form-group"> <span>Email</span>
 ...
 <script>
 	...
 	function checkExpiry() {
 		$.ajax({
 			url: "<?php echo base_url('Users/checkExpiryDate'); ?>",
 			type: 'post',
 	...
 	function addCategory() {
 		...
 		var html = '<div class="col-sm-12 col-md-12 col-lg-12 new-scheme grading_scheme_row" id="grading_scheme_row_' + rowNo + '">' +
 			...
 			'</div>' +
 			'<a class="delete" style="margin-left:15px" href="javascript:void(0)" data-rowId="' + rowNo + '"><img class="iconDltStyle" src="<?php echo base_url("assets/img/Icon_delete.png"); ?>"></a>' +
 			'</div> ' +
 			'</div>';
 	...
 	$('document').ready(function() {
 		...
 		*/
 		...
 		$('#add-email-btn').on('click', function() {
 			var student_email = $('#student_names').val();
 			if (student_email != '') {
 				if ($('span[id^="span-' + student_email + '"]').length == 0) {
 					$('#student-email-fieldset').append('<span class="email-ids" id=\'span-' + student_email + '\'>' + student_email + '<img class="cancel-email iconDtlStyle" onclick="deleteEmail(\'' + student_email + '\')" src="https://www.superrubric.com/assets/img/Icon_incorrect.png"></span>')
 					$('#student-email-fieldset').append('<input name="student_names[]" id=\'input-' + student_email + '\' type="hidden" value=\'' + student_email + '\'>')
 				} else {
 					swal("student already added")
 				}
 				//if($('#span-'+student_email).length){}

 			} else {
 				swal('student email cannot be null');
 			}
 			return false;
 		});
 	...
 	function init() {
 		...
 		$('input.criteria_1,input.criteria_2,input.criteria_3,input.criteria_4,input.criteria_5').on('blur', function() {
 			...
 			$('#avg_criteria_' + $(this).attr('data-criteria') + '_' + SchemeID).val(average.toFixed(2));
 			...
 			$('#avg_criteria_6_' + SchemeID).val(average_overall_score);
 			...
 			$("#dev_score_" + $(this).attr('data-criteria') + "_" + SchemeID).val(Std_Dev);
 			...
 			$('#dev_score_6_' + SchemeID).val(average_deviation_score);
 			...
 			$('.criteria_' + $(this).attr('data-criteria') + "_" + SchemeID).each(function() {
 				...
 				if (student_score > average) {
 					$('#' + student_criteria_analytics_id).css('color', '#37b514');
 				} else {
 					$('#' + student_criteria_analytics_id).css('color', '#ff2f2d');
 				}
 			...
 			$('.overall-score').each(function() {
 				...
 				console.log('data_cat_iddata_cat_iddata_cat_iddata_cat_id ' + data_cat_id);
 				below_up_overall_score = $(this).val() - average_overall_score;

 				if (below_up_overall_score > 0) {
 					console.log('UP TO AVERAGE');
 					$('#overall-score-below-up-student-' + $(this).attr('data-student-name') + '-' + data_cat_id).css('color', '#37b514');
 				} else {
 					console.log('BELOW TO AVERAGE');
 					$('#overall-score-below-up-student-' + $(this).attr('data-student-name') + '-' + data_cat_id).css('color', '#ff2f2d');
 				}
 				console.log(average_overall_score);
 				console.log("below_up_overall_score == " + below_up_overall_score);
 				$('#overall-score-below-up-student-' + $(this).attr('data-student-name') + '-' + data_cat_id).text(below_up_overall_score.toFixed(2));
 				$('#input-overall-score-below-up-student-' + $(this).attr('data-student-name')).val(below_up_overall_score.toFixed(2));
 			});
 			...
 			$.ajax({
 				...
 				success: function(response) {
 					if (response == 1) {
 						...
 						location.reload();
 						//setTimeout(function(){ $("#DownloadReportBtn").trigger('click'); }, 5000);
 		...
 		$('.previewButton').on('click', function() {
 			if (!$('.right-panel').hasClass("show_left_panel")) {
 				$('.wrapper .right-panel').addClass('show_left_panel');
 			} else {
 				$('.wrapper .right-panel').removeClass('show_left_panel');
 			}
 		});
 	}
 	...
 	function submitClassForm() {
 		...
 		if (check) {
 			...
 			if (sum == 100) {
 				...
 				return false;
 				$.ajax({
 					url: '<?php echo base_url('Users/checkLogin') ?>',
 					type: 'GET',
 					success: function(response) {
 						$("#ClassForm").submit();
 						if (response == 1) {
 			...
 			// icon: "https://www.superrubric.com/assets/img/Icon_incorrect.png",
 		...
 		swal("Are you sure you want to delete this class?", {
 			buttons: {
 				...
 				no: {
 					text: "Cancel",
 					value: "no",
 					className: "btn-danger class-delete-btn"
 				}
 			},
 		}).then((value) => {
 			switch (value) {
