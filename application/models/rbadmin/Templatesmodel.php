...
class Templatesmodel extends CI_Model {
	...
	public function getalltemplates($start=0,$end=100){
			
			$total_query = $this->generalmodel->custom_query('SELECT count(*) as total FROM `rb_templates` WHERE is_deleted = 0 ORDER BY `rb_templates`.`created_on` DESC');
			
						
			$query = $this->generalmodel->custom_query('SELECT * FROM `rb_templates` WHERE is_deleted = 0 ORDER BY `rb_templates`.`created_on` DESC LIMIT '.$start.','.$end);
